...
create table playsheet_situation (
	...
	foreign key (playsheet_id) references playsheet(playsheet_id) on delete cascade
);
...
create table playsheet_situation_play (
	...
	foreign key (playsheet_situation_id) references playsheet_situation(playsheet_situation_id) on delete cascade,
...
SELECT setval(
    'app_user_app_user_id_seq',
    (SELECT MAX(app_user_id) FROM app_user)
);
...
SELECT setval(
    'playbook_playbook_id_seq',
    (SELECT MAX(playbook_id) FROM playbook)
);
...
SELECT setval(
    'formation_formation_id_seq',
    (SELECT MAX(formation_id) FROM formation)
);
...
select setval(
	'playsheet_playsheet_id_seq',
	(select max(playsheet_id) from playsheet)
	);
...
SELECT setval(
    'playsheet_situation_playsheet_situation_id_seq',
    (SELECT MAX(playsheet_situation_id) FROM playsheet_situation)
);
